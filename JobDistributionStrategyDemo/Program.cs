...
using JobDistributionStrategyDemo.Consumers;
using JobDistributionStrategyDemo.Contracts;
using JobDistributionStrategyDemo.DistributionStrategies;
...
try
{
    Log.Information("Starting Job Distribution Strategy Demo");
    ...
    builder.Services.AddMassTransit(x =>
    {
        ...
        if (persistenceProvider == "mongodb")
        {
            ...
            .MongoDbRepository(r =>
            {
                ...
                r.DatabaseName = "JobDistributionDemo";
            });
        }
        else
        {
        ...
        IPublishEndpoint publishEndpoint) =>
    {
        ...
        var jobId = await publishEndpoint.SubmitJob<GenerateReport>(message, props =>
        {
            if (!string.IsNullOrEmpty(request.GroupKey))
                props.Set("GroupKey", request.GroupKey);
        }, CancellationToken.None);

        Log.Information("Report job submitted: JobId={JobId}, ReportId={ReportId}, Name={Name}, GroupKey={GroupKey}",
            jobId, reportId, request.ReportName, request.GroupKey);
        ...
        IPublishEndpoint publishEndpoint) =>
    {
        ...
        var groupKeys = new[] { "tenant-1", "tenant-2", "tenant-3", "tenant-4", "tenant-5" };
        ...
        for (int i = 0; i < request.Count; i++)
        {
            ...
            var groupKey = request.UseDifferentGroupKeys
                ? groupKeys[random.Next(groupKeys.Length)]
                : "default";
    ...
    app.MapGet("/api/reports", () =>
    {
        ...
        {
            Message = "Job Distribution Strategy Demo API",
            Endpoints = new[]
            {
                "POST /api/reports - Submit a report generation job (with optional GroupKey)",
